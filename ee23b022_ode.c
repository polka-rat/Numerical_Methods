...
#define H pow(10,-10)
#define pi M_PI
#define gold_time 0.01  //delta t of RK45 curve
#define tol 0.01    //tolerance used while computing r^2


double slope(double theta,float alpha);  //function to get the slope at a point
void r_square_h(double *x,double av1);   //function to get r^2 for heun curve
void r_square_e(double *x,double av1);   //function to get r^2 for euler curve
...
int main(int argc,char* argv[]){
	...
	double phi_slope=(gamma*H/(alpha*alpha+1));  //Expression taken from Malinson's paper
	...
	while(theta<=theta_stop){   //RK45 curve  (keeping it as the gold standard with a delta t of 0.01 seconds)
		fprintf(fp7,"%f %lf\n",rk_time,theta);
		fprintf(fp8,"%f %lf\n",rk_time,phi);
		...
		theta+=(k1+2*(k2+k3)+k4)*gold_time/6;
		theta_sum+=theta;
		theta_count++;
		rk_time+=gold_time;
		time+=delta_t;
	...
	double r2_htheta;
	r_square_h(&r2_htheta,rk_theta_avg);
	double r2_etheta;
	r_square_e(&r2_etheta,rk_theta_avg);
	
	printf("%lf %lf %lf %lf\n",alpha,delta_t,r2_etheta,r2_htheta);
	//printf("%lf %f\n",alpha,rk_time);    //used this to get the tau and alpha graph
...
void r_square_h(double *x,double av1){   //function to get r_square for Heun's curve
    double sr1=0,sr2=0,st1=0,st2=0,ye,yk;
    float t1,t2;
    FILE *fph1=fopen("HeunTheta.txt","r");
    FILE *fpr1=fopen("RK45Theta.txt","r");
    
    while(fscanf(fph1,"%f %lf",&t1,&ye)!=-1){
    	while(fscanf(fpr1,"%f %lf",&t2,&yk)!=-1){
    		if((t1-t2)<=tol){
      			sr1+=pow((ye-yk),2);
      			st1+=pow((ye-av1),2);
      			break;
    ...
    *x=((st1-sr1)/st1);  //r_square for Heun
}

void r_square_e(double *x,double av1){  //Function to get r_square for Euler's Curve
    double sr1=0,st1=0,st2=0,ye,yk;
    float t1,t2;
    FILE *fpe1=fopen("EulerTheta.txt","r");
    FILE *fpr1=fopen("RK45Theta.txt","r");

    while(fscanf(fpe1,"%f %lf",&t1,&ye)!=-1){
    	while(fscanf(fpr1,"%f %lf",&t2,&yk)!=-1){
    		if((t1-t2)<=tol){
      			sr1+=pow((ye-yk),2);
      			st1+=pow((ye-av1),2);
    ...
    *x=((st1-sr1)/st1); //r_square for Euler
